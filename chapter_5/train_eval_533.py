...
def train(config, model, train_iter, test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, y) in enumerate(train_iter):
            ...
            loss1 = loss_function(outputs[0], y[0])  # 有监督损失
            loss2 = loss_function(outputs[1], outputs[0])  # 一致性正则损失
            loss3 = loss_function(outputs[2], α*y[0] + (1-α)*y[1])  # mixup损失

            loss = a * loss1 + b * loss2 + (1-a-b) * loss3
