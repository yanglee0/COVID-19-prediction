...
class Config(object):
    ...
    def __init__(self, dataset):
        ...
        self.train_path = dataset + '/data/US/US_dataframe_train_half.csv'                                # 训练集
...
class ExternalAttention(nn.Module):
    ...
    def __init__(self, d_model, S):
        ...
        self.mk = nn.Linear(self.d_model, self.S, bias=False)
        self.mv = nn.Linear(self.S, self.d_model, bias=False)
...
class Model(nn.Module):
    ...
    def forward(self, x):      # x是train_eval中的(trains, y)中的trains
        ...
        # # 注意力机制
        # ea = ExternalAttention(d_model=100, S=4)
        # hid = ea(hid)
