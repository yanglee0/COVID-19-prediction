...
class Config(object):
    ...
    def __init__(self, dataset):
        self.model_name = 'rnn'
        self.train_path = dataset + '/data/IN/IN_dataframe_train.csv'                                # 训练集
        self.test_path = dataset + '/data/IN/IN_dataframe_test.csv'                                  # 测试集
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.rnn = nn.RNN(config.input_size, config.hidden_layer_size, batch_first=True)
    ...
    def forward(self, x):
        ...
        lstm_out, self.hidden_cell = self.rnn(x)   # hidden不用写  输出维度为 1*7*1
